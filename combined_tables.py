...
for header in required_during_RFQ.keys():
    html_table_second += f"          <th>{header}</th>\n"
...
"""
    ...
    for header in required_during_RFQ.keys():
        html_table_second += f"          <td>{required_during_RFQ.get(header, '')}</td>\n"

    # Add data for "Service Provider Reply"
    for header in service_provider_reply.keys():
        html_table_second += f"          <td>{service_provider_reply.get(header, '')}</td>\n"

    html_table_second += "        </tr>\n"
...
with open("combined_tables.html", "w") as file:
    file.write(html_table_first)

with open("combined_tables.html", "a") as file:
    file.write(html_table_second)
